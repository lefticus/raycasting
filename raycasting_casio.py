...
def in_range(min_, max_, value):
  return (min_ - .0000001) <= value <= (max_ + .0000001)
...
def intercept(l1: Line, l2: Line):
    ...
    # just plug it back into one of the line forumals and get the answer!
...
def intersecting_segments(input_: Segment, segments):
    ...
    for segment in segments:
        if not(segment.min_x < input_.max_x and segment.max_x > input_.min_x and segment.min_y < input_.max_y and segment.max_y > input_.min_y):
            continue
...
def make_map(map):
    ...
    print(f"Filtered dups: {len(result)}")
        ...
        for s in result:
            for n in result:
                if s.end.x == n.start.x and s.end.y == n.start.y and n.slope == s.slope:
                    remove_list += [n, s]
                    result.append(Segment(Point(s.start.x, s.start.y), Point(n.end.x, n.end.y)))
                    break;
                elif not s is n and  s.end.x == n.end.x and s.end.y == n.end.y and n.slope == s.slope:
                    remove_list += [n, s]
                    result.append(Segment(Point(s.start.x, s.start.y), Point(n.start.x, n.start.y)))
                    break;
                elif not s is n and  s.start.x == n.start.x and s.start.y == n.start.y and n.slope == s.slope:
                    remove_list += [n, s]
                    result.append(Segment(Point(s.end.x, s.end.y), Point(n.end.x, n.end.y)))
                    break;
...
c = Camera(Point(-.5, -.5), math.pi / 2, math.pi / 2)
...
while True:
    ...
    for r in c.rays(width):
        matches = intersect_ray(r, segments)
        ...
        if len(matches) > 0 and matches[0][0] != 0:
            wall_height = ((height * .75) / matches[0][0]) # Distance
